...
static void write_root_signature(FILE *output, descriptor_set *all_descriptor_sets[256], size_t all_descriptor_sets_count) {
	...
	fprintf(output, "\tD3D12_ROOT_PARAMETER params[%i] = KONG_INIT_ZERO;\n", table_count);
	...
	for (size_t set_index = 0; set_index < all_descriptor_sets_count; ++set_index) {
		...
		if (has_other) {
			...
			fprintf(output, "\n\tD3D12_DESCRIPTOR_RANGE ranges%i[%zu] = KONG_INIT_ZERO;\n", table_index, count);
	...
	fprintf(output, "\n\tD3D12_ROOT_SIGNATURE_DESC desc = KONG_INIT_ZERO;\n");
...
void kore3_export(char *directory, api_kind api) {
	...
	type_id vertex_inputs[256]              = KONG_INIT_ZERO;
	size_t  vertex_input_slots[256]         = KONG_INIT_ZERO;
	bool    vertex_inputs_per_instance[256] = KONG_INIT_ZERO;
...

	{
		...
		char    root_constants_type_name[256] = KONG_INIT_ZERO;
		...
		for (type_id i = 0; get_type(i) != NULL; ++i) {
			type *t = get_type(i);
			if (!t->built_in && has_attribute(&t->attributes, add_name("pipe"))) {
				fprintf(output, "\tkore_%s_render_pipeline_parameters %s_parameters = KONG_INIT_ZERO;\n\n", api_short, get_name(t->name));
				...
				type_id vertex_inputs[64]   = KONG_INIT_ZERO;
				bool    instanced[64]       = KONG_INIT_ZERO;
		...
		for (type_id i = 0; get_type(i) != NULL; ++i) {
			type *t = get_type(i);
			if (!t->built_in && has_attribute(&t->attributes, add_name("raypipe"))) {
				fprintf(output, "\tkore_%s_ray_pipeline_parameters %s_parameters = KONG_INIT_ZERO;\n\n", api_short, get_name(t->name));
