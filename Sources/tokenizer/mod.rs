...
pub fn tokenize(source: &str) -> Vec<Token> {
	...
	let mut state = State::new(&mut it);
		...
		match state.next {
			None => {
				match mode {
					...
					Mode::Stringy => panic!("Unclosed string"),
					Mode::Operator => panic!("File ends with an operator"),
					Mode::Comment => panic!("Unclosed comment"),
				}
				break tokens;
			}
			Some(ch) => {
				match mode {
					Mode::Select => {
						if ch == '/' {
							if let Some(next_ch) = state.next_next {
								mode = match next_ch {
									'/' => Mode::LineComment,
									'*' => Mode::Comment,
									_ => {
					...
					Mode::Comment => {
						if ch == '*' {
							if let Some(ch) = state.next_next {
								if ch == '/' {
									mode = Mode::Select;
									state.advance();
								}
					...
					Mode::Identifier => {
						if is_op(ch) || ch == ' ' || ch == '(' || ch == ')' || ch == '{' || ch == '}' || ch == '"' || ch == '\'' || ch == ';' || ch == '.' || ch == ',' || ch == ':' || ch == '\n' || ch == '\r' {
							push_identifier(&mut tokens, &buffer);
							mode = Mode::Select;
						}
						else {
